...
const WomenShoes = () => {
  ...
  // Access user location, exchange rate, and currency from CurrencyContext
  ...
  //         setCurrency('LKR');
  //         setExchangeRate(1); // No conversion for LKR
  //       } else {
  //         setCurrency('USD');
  //         const rate = await fetchExchangeRate();
  //         setExchangeRate(rate || 1.5); // Fallback exchange rate
  //       }
  //     } catch (error) {
  ...
  // const convertCurrency = (priceInLKR) => {
  //   return (priceInLKR / exchangeRate).toFixed(2);
  // };
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      if (!userLocation) return; // Wait for userLocation to be set
      ...
      try {
        ...
        const filteredProducts = productsData.filter(product =>
          ...
          // Include Women, Men & Women, and Shoes categories
      ...
      {!loading && (
        <div className='men-main-container'>
          ...
          <p className='men-breadcrumb'>
            <Link to='/'>HOME</Link> <i className="fas fa-angle-right" /> <Link to="/women-shoes">WOMEN SHOES</Link><i className="fas fa-angle-right" />
          </p>
          <Filter
